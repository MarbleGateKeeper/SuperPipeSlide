...
public class BranchAnchorBlock extends Block {
    ...
    protected void onPlace(BlockState state, Level level, BlockPos pos, BlockState oldState, boolean movedByPiston) {
        if (!level.isClientSide() && level instanceof ServerLevel serverLevel && !state.is(oldState.getBlock())) {
            PipeNetworkSavedData.get(serverLevel).initializeBranchNode(PipeAnchorId.of(level, pos));
        }
    ...
    protected void affectNeighborsAfterRemoval(BlockState state, ServerLevel level, BlockPos pos, boolean movedByPiston) {
        PipeNetworkSavedData.get(level).removeBranchNodeAndConnections(PipeAnchorId.of(level, pos));
