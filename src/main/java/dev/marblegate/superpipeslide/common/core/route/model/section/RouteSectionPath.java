...
public record RouteSectionPath(List<PipeConnectionRef> forwardConnections, List<PipeConnectionRef> reverseConnections) {
    ...
    private static final StreamCodec<RegistryFriendlyByteBuf, List<PipeConnectionRef>> CONNECTION_REF_LIST_CODEC =
            PipeConnectionRef.STREAM_CODEC.apply(ByteBufCodecs.list(MAX_CONNECTIONS));
    ...
    public static final Codec<RouteSectionPath> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            PipeConnectionRef.CODEC.listOf().optionalFieldOf("reverse_connections", List.of()).forGetter(RouteSectionPath::reverseConnections)
    ).apply(instance, RouteSectionPath::new));
